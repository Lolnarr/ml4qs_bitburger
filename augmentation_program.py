...
def add_noise(df: pd.DataFrame):
    mu, sigma = 0, 1.0
    noise = np.random.normal(mu, sigma, [500, 3])
    fusion = df[['roll', 'pitch', 'yaw']] + noise
    return fusion
...
def start_augmentation(path: str, rotate=True, gyronoise=False, noise=True, gyrostretch=False, stretch=True):
